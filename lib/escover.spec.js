...
test('escover: initialize: port', async (t) => {
    mockImport('./exit', {
        exit: stub(),
    });
    
    const {initialize} = await reImport('./escover.js');
    const port = {};
    ...
    t.ok(port.onmessage, 'should set port.onmessage');
    t.end();
});

test('escover: initialize: void', async (t) => {
    mockImport('./exit', {
        exit: stub(),
    });
