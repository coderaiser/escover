...
export function groupByFolder(files) {
    ...
    for (const f of files) {
        ...
        group.files.push({ ...f, shortName: fileName });
    }
...
export function buildGroupedTable(files, showPercent, linesColWidth) {
    ...
    if (showPercent)
        tableData.push(['File', '% Lines', 'Uncovered Lines #s']);
    else
        tableData.push(['File', 'Uncovered Lines #s']);

    const groups = groupByFolder(files);
    const hideFolders = groups.size === 1;

    const getColor = (value) => value === 100 ? makeGreen : makeRed;

    for (const [folder, group] of groups) {
        let sum = 0;
        for (const f of group.files) sum += f.percentLines;
        const coverage = Math.round(sum / group.files.length);

        if (!hideFolders) {
            const row = [];
            row.push(getColor(coverage)(folder));
            if (showPercent) row.push(getColor(coverage)(`${coverage}%`));
            row.push('');
            tableData.push(row);
        }
        ...
        for (const f of group.files) {
            ...
            if (showPercent) row.push(f.percentLines === 100 ? makeGreen(`${f.percentLines}%`) : makeRed(`${f.percentLines}%`));
            row.push(f.covered ? '' : makeRed(formatLines(f.lines, linesColWidth)));
            tableData.push(row);
        }
    }
...
export function formatLines(array, maxLen = 20) {
    ...
    for (let i = 1; i < nums.length; i++) {
        const n = nums[i];
        if ((typeof prev === 'number' && typeof n === 'number' && n === prev + 1) || (prev === n - 0)) prev = n;
        else {
            ranges.push(`${start}`);
            start = n;
            prev = n;
...
export function parseUncoveredLines(lines) {
    const out = [];
    for (const [line, covered] of entries(lines)) if (!covered) out.push(line);
    return out;
}

export function getLinesPercent(linesCount, uncoveredLinesCount) {
    if (!linesCount) return 100;
