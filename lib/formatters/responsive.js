...
export function buildGroupedTable(files, showPercent) {
    ...
    for (const [folder, group] of groups) {
        ...
        if (!hideFolders) {
            ...
            row.push(getColor(coverage)(trim(folder, 25)));
        ...
        for (const f of group.files) {
            ...
            if (f.covered)
                row.push(makeGreen(fileCell));
            else
                row.push(makeRed(fileCell));
            
            if (showPercent)
                if (f.percentLines === 100)
                    row.push(makeGreen(`${f.percentLines}%`));
                else
                    row.push(makeRed(`${f.percentLines}%`));
            
            if (!f.covered)
                row.push(makeRed(formatLines(f.lines)));
            else
                row.push('');
