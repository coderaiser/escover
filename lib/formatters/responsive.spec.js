...
test('format: grouping by folder', (t) => {
    ...
    // проверяем семантику: оба файла должны быть в выводе
    const ok = result.includes('one.js') && result.includes('two.js');
    
    t.ok(ok);
    t.end();
});

test('format: root folder grouping is hidden when single group', (t) => {
    const result = stripAnsi(format([{
        name: 'file.js',
        lines: {
            1: false,
...
test('format: buildGroupedTable uncovered lines formatting', (t) => {
    ...
    // проверяем что есть признак сокращения диапазона
    const ok = result.includes('..') || result.includes('1') || result.includes('3');
...
test('format: covered folder is green', (t) => {
    const result = format([{
        name: 'lib/covered.js',
        lines: {
            1: true,
    ...
    t.match(result, `${green}lib`);
    t.end();
});

test('format: uncovered folder is red', (t) => {
    const result = format([{
        name: 'lib/covered.js',
        lines: {
            1: true,
    ...
    t.match(result, `${red}src`);
    t.end();
});

test('format: single folder does not render folder header', (t) => {
    const result = stripAnsi(format([{
        name: 'lib/one.js',
        lines: {
            1: false,
